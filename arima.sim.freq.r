arima.sim.freq <- function(n, model, smodel, D, SD, std, fs, unit)
  ...
  if (is.na(DT_Poles)==1){DT_Poles <- DT_SPoles} else if (is.na(DT_SPoles)==1) {DT_Poles <- DT_Poles} else {DT_Poles <- c(DT_Poles,DT_SPoles)}
  if (is.na(DT_Zeros)==1){DT_Zeros <- DT_SZeros} else if (is.na(DT_SZeros)==1) {DT_Zeros <- DT_Zeros} else {DT_Zeros <- c(DT_Zeros,DT_SZeros)}
  } else {DT_SPoles <- NA; DT_SZeros <- NA}
  DT_Poles <- unique(DT_Poles); DT_Zeros <- unique(DT_Zeros);
  if (is.na(DT_Poles)==0){A <- as.polynomial(rev(poly.calc(DT_Poles))); tau = log(1/10)/log(max(abs(DT_Poles)))/fs;} else {A <- as.polynomial(1); tau <- 1}
  if (is.na(DT_Zeros)==0){B <- as.polynomial(rev(poly.calc(DT_Zeros)))} else {B <- as.polynomial(1)}
